    ...
    def fill_attendance():
        ...
        if subb=='':
            ...
            def create_csv():
                ...
                csv_name='C:/Users/Shruti Goel/Desktop/Attendace_management_system-master/Attendance/Manually Attendance'+DB_table_name+'.csv'
            ...
            def attf():
                import subprocess
                subprocess.Popen(r'explorer /select,"C:/Users/Shruti Goel/Desktop/Attendace_management_system-master/Attendance/Manually Attendance/-------Check atttendance-------"')
...
#recognizer.train(faces, Id.reshape(-1, 1))

# ...

def Fillattendances(Id):
    global ids, User
    if Id == -1:
        print("No user detected")
    else:
        User = Ids[np.where(Ids == Id)[0][0]]
        print(f"User {User} Detected")
        ids.append(User)
        User = []
        Id = []
...
#recognizer.train(faces, Ids.reshape(-1, 1))
...
def subjectchoose():
        ...
        if time.time() < future:
            if sub == '':
                ...
                while True:
                    ...
                    for (x, y, w, h) in faces:
                        global Id
                        #os.remove('TrainingImageLabels.yml')
                        recognizer.save('TrainingImageLabels.yml')
                        if os.path.isfile('TrainingImageLabels.yml'):
                             os.remove('TrainingImageLabels.yml')
                        recognizer.save('MyTrainedRecognizer.yml')
                        Id, conf = recognizer.id, confidence = recognizer.recognize(gray[y:y + h, x:x + w])
                        if (conf <70):
                            print(conf)
                            global Subject
                            global aa
                            ...
                            if 'Enrollment' in df.columns:
                                ...
                                print("Column 'Enrollment' not found in DataFrame")
                            aa = df.loc[df['Enrollment'] == Id]['Name'].values
                            global tt
                            tt = str(Id) + "-" + aa
                            En = '15624031' + str(Id)
                ...
                root.configure(background='snow')
                cs = 'C:/Users/Shruti Goel/Desktop/Attendace_management_system-master'+fileName
    ...
    def Attf():
        import subprocess
        subprocess.Popen(r'explorer /select,"C:\Users\Shruti Goel\Desktop\Attendace_management_system-master\Attendance\-------Check atttendance-------"')
    ...
    def log_in():
        ...
        if username == 'admin' :
            if password == 'admin1234':
                ...
                import csv
                rows = []
                with open("C:/Users/Shruti Goel/Desktop/Attendace_management_system-master/StudentDetails/StudentDetails.csv", 'r') as file:
                ...
                print(header)
                print(rows)
                with open('C:/Users/Shruti Goel/Desktop/Attendace_management_system-master/StudentDetails/StudentDetails.csv') as file:
                     content = file.readlines()
                header = content[:1]
                rows = content[1:]
                print(header)
                print(rows)
                cs = 'C:/Users/Shruti Goel/Desktop/Attendace_management_system-master/StudentDetails/StudentDetails.csv'
                with open(cs, newline="") as file:
                    reader = csv.reader(file)
                    r = 1
                    ...
                    for col in reader:
                        ...
                        for row in col:
                            label = tkinter.Label(root, width=8, height=1, fg="black", font=('times', 15, ' bold '),
                                                  bg="lawn green", text=row, relief=tkinter.RIDGE)
                            label.grid(row=r, column=c)
                            data= pd.read_csv("C:/Users/Shruti Goel/Desktop/Attendace_management_system-master/StudentDetails/StudentDetails.csv")
                            data
                            c = c+1
                        r += 1
                root.mainloop()
